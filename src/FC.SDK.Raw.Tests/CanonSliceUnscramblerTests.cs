...
/// (with the real EOS 6D file) is gated behind <c>FC_SDK_RAW_TEST_CR2</c> in
/// <see cref="Cr2EndToEndTests"/>.
/// </summary>
public class CanonSliceUnscramblerTests
{
    ...
    public void SingleSlice_IsIdentity()
    {
        ...
        var result = CanonSliceUnscrambler.Unscramble(samples, outputWidth:4, outputHeight:3, sliceCount: 1, sliceWidth: 4, lastSliceWidth: 4);
    ...
    public void TwoEqualSlices_InterleavesByRow()
    {
        ...
        //   slice 0 = [A0, A1, A2, A3]  (s0_r0c0, s0_r0c1, s0_r1c0, s0_r1c1)
        //   slice 1 = [B0, B1, B2, B3]
        // Expected output (row-major):
        //   [A0, A1, B0, B1,
        //    A2, A3, B2, B3]
        ushort[] samples = [0xA0, 0xA1, 0xA2, 0xA3, 0xB0, 0xB1, 0xB2, 0xB3];

        var result = CanonSliceUnscrambler.Unscramble(samples, outputWidth:4, outputHeight:2, sliceCount: 2, sliceWidth: 2, lastSliceWidth: 2);
    ...
    public void NarrowerLastSlice_HandlesAsymmetricLayout()
    {
        ...
        var result = CanonSliceUnscrambler.Unscramble(samples, outputWidth:5, outputHeight:2, sliceCount: 2, sliceWidth: 3, lastSliceWidth: 2);
    ...
    public void SampleCountMismatch_Throws()
    {
        ...
        Should.Throw<System.IO.InvalidDataException>(() =>
            CanonSliceUnscrambler.Unscramble(samples, outputWidth:5, outputHeight:2, sliceCount: 1, sliceWidth: 5, lastSliceWidth: 5));
    }
    ...
    public void EosLike_TwoSlicesOf2784_FillsCorrectColumns()
    {
        ...
        // Synthetic samples: each sample equals (slice_idx * 100_000_000) + row * 10000 + col_in_slice
        // so we can verify the layout deterministically without needing the real JPEG.
        ...
        for (var slice = 0; slice < 2; slice++)
        {
            for (var y = 0; y < H; y++)
            {
                ...
                {
                    // Hash to 16-bit so the value still uniquely identifies (slice, y, x_in_slice).
                    samples[idx++] = (ushort)((slice * 0x4000) ^ (y * 31) ^ x);
                }
            }
        }

        var result = CanonSliceUnscrambler.Unscramble(samples, W, H, sliceCount: 2, sliceWidth: SLICE_W, lastSliceWidth: SLICE_W);

        // Spot-check the corners: output[y][x] for known (slice, y, x_in_slice).
        ...
        result[SLICE_W].ShouldBe(Expected(1, 0, 0));        // first column of slice 1
        result[W - 1].ShouldBe(Expected(1, 0, SLICE_W - 1)); // last column of row 0
